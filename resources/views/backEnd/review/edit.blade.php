                ...
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('reviews.update') }}" method="POST" class="row" data-parsley-validate=""
                            ...
                            <div class="col-sm-6">
                                <div class="form-group mb-3">
                                    <label for="customer_id" class="form-label">Customer Select *</label>
                                    <select class="form-control select2-multiple @error('customer_id') is-invalid @enderror"
                                        ...
                                        <optgroup>
                                            ...
                                            @foreach ($customers as $value)
                                                <option value="{{ $value->id }}"
                                                    {{ $edit_data->customer_id == $value->id ? 'selected' : '' }}>
                                                    {{ $value->name }}</option>
                                            @endforeach
                                        </optgroup>
                                    </select>
                            ...
                            <div class="col-sm-4">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="number" class="form-control @error('ratting') is-invalid @enderror"
                                        name="ratting" value="{{ $edit_data->ratting }}" id="ratting" required="">
                                    @error('ratting')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                            ...
                            <div class="col-sm-4 mb-3">
                                <div class="form-group">
                                    ...
                                    <textarea name="review"  class="summernote form-control @error('review') is-invalid @enderror" required>{{ $edit_data->review }}</textarea>
                                    @error('review')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
